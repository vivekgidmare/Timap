...
public class TiUIMapView extends TiUIFragment implements
	...
	protected void addAnnotation(AnnotationProxy annotation) {
		...
		System.out.println("AddAnnotation has been called");
		...
		System.out
				.println("title :" + annotation.getMarkerOptions().getTitle());
	...
	protected void removeAllAnnotations() {
		for (int i = 0; i < timarkers.size(); i++) {
			...
			timarker.setAnnotationInfo(null);// added by vivek
			AnnotationProxy proxy = timarker.getProxy();
			if (proxy != null) {
				proxy.setTiMarker(null);
				proxy.setAnnotationInfo(null);
			}
		}
		timarkers.clear();
	}
	...
	protected void removeAnnotation(Object annotation) {
		...
		if (timarker != null && timarkers.remove(timarker)) {
			...
			if (proxy != null) {
				proxy.setTiMarker(null);
				proxy.setAnnotationInfo(null);//added by vivek	
			}
		}
	}
	...
	public void onInfoWindowClick(Marker marker) {
		System.out.println("onInfoWindowClick called");

		AnnotationProxy annoProxy = getProxyByMarker(marker);
		if (annoProxy != null) {
			// String clicksource =
			// annoProxy.getMapInfoWindow().getClicksource();
			String clicksource = annoProxy.getTiMarker().getMarker().getTitle();
			System.out.println("Clicked :" + clicksource);
	...
	public View getInfoContents(Marker marker) {
		infoWindowAdapter = new TiInfoWindowAdapter(marker);
